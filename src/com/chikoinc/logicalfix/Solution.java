package com.chikoinc.logicalfix;

/**
 * 
 * @author Chiko CyclicRotation: Rotate an array to right by a given number of
 *         steps
 * 
...
public class Solution {
	public int[] solution(int[] A, int K) {
		int[] solutionArray = A;
		for (int k = 0; k < K; k++) {
			int temp = solutionArray[A.length - 1];
			for (int i = A.length - 1; i > 0; i--) {
				solutionArray[i] = solutionArray[i - 1];
			}
			solutionArray[0] = temp;
		}
	...
	public static void main(String[] args) {
		checkSolution(new int[] { 3, 8, 9, 7, 6 }, 3);
		checkSolution(new int[] { 1, 2, 3, 4 }, 3);
		checkSolution(new int[] { 0, 0, 0 }, 3);
	}

	public static void checkSolution(int[] A, int K) {
		Solution s = new Solution();

		int[] solvedArray = s.solution(A, K);
		for (int i = 0; i < solvedArray.length; i++) {
			System.out.print(solvedArray[i]);
		}
		System.out.println("");
	}
}